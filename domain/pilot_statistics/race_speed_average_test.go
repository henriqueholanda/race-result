...
func TestShouldReturnPilotRaceSpeedAverage(t *testing.T) {
	...
	raceSpeedAverage := pilot_statistics.NewRaceSpeedAverage()

	pilotStatistics := models.RacePilotStatistics{
		LapAmount: len(raceStatistics),
	}

	assert.Equal(t, 240.5, raceSpeedAverage.Generate(raceStatistics, pilotStatistics).SpeedRaceAverage)
}
